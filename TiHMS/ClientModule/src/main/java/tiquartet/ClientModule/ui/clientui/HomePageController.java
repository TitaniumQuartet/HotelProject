...
public class HomePageController implements Initializable {
	...
	void onSearchHotel(ActionEvent event) {
		if(districtBox.getSelectionModel().getSelectedIndex()>=0){
			try {
				...
				List<HotelBriefVO> hotelList = HMSClient.getSearchHotelBL().getHotelList(filterVO, HotelSort.评分降序, 1, 6);
	...
	public void setContent() {
		try {
			...
			for (int i = 0; i < 3; i++)
				orderGridPane.add(new OrderPane(orderList.get(i)), 0, i);
		} catch (RemoteException e) {
			// 处理网络异常
			e.printStackTrace();
		}
		...
		cityBox.getSelectionModel().selectedIndexProperty()
				.addListener(new ChangeListener<Number>() {
					...
					public void changed(
						...
						else districtBox.setDisable(false);
