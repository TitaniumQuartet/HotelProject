...
public class SearchHotelController implements Initializable{
    ...
    public void refresh(){
    	...
    	try {
    		int num=(total>=0&&total<page*10)?(total-page*10+10):10;
			list=HMSClient.getSearchHotelBL().getHotelList(filter, sort, page*10-9, page*10-9+num-1);
			total = list.size();
			...
			if(total>10){
				...
				for(int i=1;i<=Math.ceil(total/10.0);i++) pageNumBox.getItems().add(i);
			}
			...
			for(int i=page*10-9;i<=page*10-9+num-1;i++){
				grid.add(new HotelBriefPane(list.get(i-1)), 1-i%2, (i-1)/2);
			}
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		starBox.getSelectionModel().selectedIndexProperty().addListener(new ChangeListener<Number>() {
			@Override
		...
		rateBox.getSelectionModel().selectedIndexProperty().addListener(new ChangeListener<Number>() {
		...
		priceBox.getSelectionModel().selectedIndexProperty().addListener(new ChangeListener<Number>() {
		...
		pageNumBox.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<Integer>() {

			@Override
			public void changed(ObservableValue<? extends Integer> observable,
					Integer oldValue, Integer newValue) {
				if(newValue>0) page=newValue;
				refresh();
			}
		});
	}
