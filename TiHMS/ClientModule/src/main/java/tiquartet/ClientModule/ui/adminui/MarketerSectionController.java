...
public class MarketerSectionController implements Initializable{
    ...
    void onAddMarketer(ActionEvent event) {
    	...
    	if(addMarketerButton.getText().equals("确认")){
    		try {
				...
				if(message.result){
					hideAddMarketer();
					Alert success = new Alert(AlertType.INFORMATION, "营销人员添加成功！");
					success.show();
					...
					Alert fail = new Alert(AlertType.ERROR, "营销人员添加失败，"+message.failInfo);
					fail.show();
				}
			} catch (RemoteException e) {
				//网络连接异常处理
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		//需要调用RMI服务
		/*try {
			users = HMSClient.getManageUserBL().marketerList();
		} catch (RemoteException e) {
			// 网络异常处理
			e.printStackTrace();
		}*/
		...
		marketerListTable.getSelectionModel().setSelectionMode(SelectionMode.SINGLE);
		marketerListTable.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<UserVO>() {
