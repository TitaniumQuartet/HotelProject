...
public class StrategyPane extends FlowPane {
	...
	public StrategyPane(StrategyVO vo) {
		...
		if (vo.strategyID == 1) {
			...
			introLabel = new Label(intro);
			introLabel.setFont(new Font(14));
			introLabel.setPrefWidth(605);
		...
		typeLabel = new Label("策略类型：" + vo.strategyType.name());
		typeLabel.setFont(new Font(18));
		getChildren().add(typeLabel);
		introLabel = new Label(vo.strategyIntro);
		introLabel.setFont(new Font(14));
		introLabel.setPrefWidth(605);
		introLabel.setWrapText(true);
		getChildren().add(introLabel);
				...
				if (modifyButton.getText().equals("修改")) {
					switch (vo.strategyType) {
						case BIRTHDAY :
							break;
						case CIRCLE :
							int index = getChildren().indexOf(districtLabel);
							getChildren().remove(index);
							cityBox = new ChoiceBox<>();
							districtBox = new ChoiceBox<>();
							districtBox.setDisable(true);
							cityBox.getItems()
									.addAll(DistrictData.getCityMap().values());
					...
					switch (vo.strategyType) {
						...
						case TIME :
							vo.startTime = startDatePicker.getValue()
									.toString();
							vo.endTime = endDatePicker.getValue().toString();
							startDatePicker.setDisable(true);
							endDatePicker.setDisable(true);
							break;
						default :
					...
					vo.discount = Double.parseDouble(discountField.getText())
							/ 100.0;
					discountField.setEditable(false);
