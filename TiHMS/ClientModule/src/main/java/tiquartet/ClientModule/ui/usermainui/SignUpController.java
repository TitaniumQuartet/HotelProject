...
public class SignUpController implements Initializable {
	...
	private void checkSignUp() {
		...
		} else if (l != 0 && (l < 6 || l > 16)) {
			// 若输入的用户名长度不正确
			usernamePrompt.setText("* 6-16个字符");
		} else {
			try {
				if (!HMSClient.getUserMainBL()
						.isUnregistered(newUsernameField.getText())) {
					// 若该用户名已被注册
		...
		// 检查密码确认输入
		if (!confirmPasswordField.getText()
				.equals(newPasswordField.getText())) {
			// 若确认密码内容与密码不一致
		...
		// 三个输入均正确时按钮可用
		if (usernameValid && passwordValid && confirmpwValid)
			finishSignUpButton.setDisable(false);
	}

	@Override
	public void initialize(URL location, ResourceBundle resources) {
		newUsernameField.focusedProperty()
		...
		newPasswordField.focusedProperty()
				.addListener(new ChangeListener<Boolean>() {
					@Override
					public void changed(
							ObservableValue<? extends Boolean> observable,
