...
public class CreditDataSqlHelper implements CreditDataHelper {
	...
	public ResultMessage insert(CreditPO creditItem) {
		...
		String sql = "insert into credit(changeType,addition,balance,orderId,userId,time) values(?,?,?,?,?,?)";
		...
		try {
			...
			while (rs.next()) {
				...
				CreditPO creditpo = new CreditPO(
						CreditChange.values()[changeType], change, balance,
						orderId, creditRecordId, userId, time);
				credit.add(creditpo);
			}
			pstmt.close();
			conn.close();
